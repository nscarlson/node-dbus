    ...
    if (!enabledSignal) {
        ...
        _dbus.setSignalHandler(function (uniqueName) {
            if (DBus.signalHandlers[uniqueName]) {
                var args = ['signal'].concat(
                    Array.prototype.slice.call(arguments),
                )

                DBus.signalHandlers[uniqueName].emit.apply(
                    DBus.signalHandlers[uniqueName],
                    args,
                )
            }
        })
...
DBus.registerService = function (busName, serviceName) {
    ...
    if (serviceName) {
        ...
        if (serviceMap[serviceHash]) return serviceMap[serviceHash]
    }

    // Get a connection
    var bus = DBus.getBus(busName)

    if (!serviceName) _serviceName = bus.connection.uniqueName
