...
var ServiceObject = (module.exports = function (service, objectPath) {
    ...
    self.introspectableInterface = self.createInterface(
        'org.freedesktop.DBus.Introspectable',
    )
    self.introspectableInterface.addMethod(
        'Introspect',
    ...
    function DefineByPropName(name) {
        return (_, __, args) => {
            ...
            const iface = self.interfaces[interfaceName]
    ...
    self.propertyInterface = self.createInterface(
        'org.freedesktop.DBus.Properties',
    )
    self.propertyInterface.addMethod(
        'Get',
        {
            in: [
...
ServiceObject.prototype.createInterface = function (interfaceName) {
    ...
    if (!self.interfaces[interfaceName])
        self.interfaces[interfaceName] = new ServiceInterface(
            self,
            interfaceName,
        )
...
ServiceObject.prototype.buildChildNodes = function () {
    ...
    var childKeys = allKeys.filter(function (key) {
        return (
            key.substr(0, prefix.length) == prefix && key.length > prefix.length
        )
    })
