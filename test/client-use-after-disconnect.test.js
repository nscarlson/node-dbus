        ...
        setTimeout(function () {
            iface.getProperty('Test', function (err, value) {
                ...
                tap.match(
                    err.message,
                    /no.*connected/,
                    'getProperty returns the correct error',
                )
            })
            iface.setProperty('Test', 'Value', function (err) {
                ...
                tap.match(
                    err.message,
                    /no.*connected/,
                    'setProperty returns the correct error',
                )
            })
            iface.getProperties(function (err, values) {
                ...
                tap.match(
                    err.message,
                    /no.*connected/,
                    'getProperties returns the correct error',
                )
            })
            iface.NameHasOwner('Test', function (err, result) {
                ...
                tap.match(
                    err.message,
                    /no.*connected/,
                    'call returns the correct error',
                )
            })
            bus.getInterface(
                'org.freedesktop.DBus',
                ...
                function (err, iface) {
                    ...
                    tap.match(
                        err.message,
                        /no.*connected/,
                        'getInterface returns the correct error',
                    )
                },
            )
        }, 100)
