...
function checkInterface(bus, value, callback) {
    ...
    bus.getInterface(
        'test.dbus.DynamicTestService',
        '/test/dbus/DynamicTestService/subs/' + value,
        'test.dbus.DynamicTestService.DynamicInterface1',
        function (err, iface) {
            if (!err) {
                ...
                iface.getProperty('Value', function (err, result) {
                    ...
                    if (
                        err instanceof DBus.Error &&
                        /UnknownMethod/.test(err.dbusName)
                    ) {
